...
- (id)initWithImageView:(UIImageView *)imageView
{
    ...
    if (self) {
        ...
        self.pointColor      = [UIColor lightGrayColor];
...
- (void)addPoints:(int)num
{
    ...
    UIView *point;
    if (IS_HEIGHT_480) {
        point = [self getPointView:pointsAdded at:CGPointMake(20, 100)];
    }
    else
    {
        point = [self getPointView:pointsAdded at:CGPointMake(50, 100)];
    }
    ...
    // Corner 2
    if (IS_HEIGHT_480) {
        point = [self getPointView:pointsAdded at:CGPointMake(self.frame.size.width -20, 100)];
    }
    else
    {
        point = [self getPointView:pointsAdded at:CGPointMake(self.frame.size.width -50, 100)];
    }
    ...
    // Corner 3
    if (IS_HEIGHT_480) {
        point = [self getPointView:pointsAdded at:CGPointMake(self.frame.size.width -20, self.frame.size.height -170)];

    }
    else
    {
        point = [self getPointView:pointsAdded at:CGPointMake(self.frame.size.width -50, self.frame.size.height -100)];
    }
    ...
    // Corner 4
    if (IS_HEIGHT_480) {
        point = [self getPointView:pointsAdded at:CGPointMake(20, self.frame.size.height -170)];
    }
    else
    {
        point = [self getPointView:pointsAdded at:CGPointMake(50, self.frame.size.height -100)];
    }
    [tmp addObject:point];
    [self addSubview:point];
    pointsAdded ++;
...
- (UIView *)getPointView:(int)num at:(CGPoint)point
{
    ...
    point1.layer.cornerRadius = k_POINT_WIDTH+5;
...
-(UIImage *)getCroppedImageForView:(UIImageView *)view withTransparentBorders:(BOOL)transparent{
    ...
    if(!transparent)
    {
        ...
        [[(AppDelegate *)[[UIApplication sharedApplication] delegate] navigation] pushViewController:VC animated:NO];
    ...
    else
    {
        ...
        return [self cropTransparencyFromImage:image];
    }
